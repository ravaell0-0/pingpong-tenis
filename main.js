class Ball {
  ...
  class Paddle {
    ...
    autoMove(ballY) {
      ...
      if (diff < 0 && diff < -this.height * 0.5) { // gerak ke bawah
        this.switchMoveDirection('down');
      } else if (diff > 0 && diff > this.height * 0.5) { // gerak ke atas
  ...
  function setup() {
    setupCanvas();
    leftPlayerScore = 0; rightPlayerScore = 0; // atur papan skor
  ...
  function setupCanvas() {
    ...
    // jaga dimensi lapangan tetap rapi
    width = min(window.innerWidth, height * 2);
    createCanvas(width, height);
  }
  
  function draw() {
    paddleOne.move(); // gerakkan pemain sesuai input keyboard
    paddleTwo.autoMove(ball.y); // pemain komputer bergerak sendiri relatif terhadap posisi bola
    updateBall();
    isColliding(); // periksa apakah bola sedang bertabrakan dengan pemain
    didHitWall(); // periksa apakah bola sedang bertabrakan dengan dinding
    didScore(); // periksa apakah seorang pemain telah mencetak poin
    background(255); // gambar latar
    drawCourt();
    drawCourtLines();
    paddleOne.render(); // gambar pemain
    paddleTwo.render(); // gambar pemain komputer
    drawNet();
    ball.render(); // gambar bola
    drawScore();
  }
  ...
  function isColliding() {
    ...
    if (ball.x >= paddleOne.x - paddleOne.width && ball.x <= paddleOne.x + paddleOne.width) {
      // jika bola berada di rentang paddleOne
      ...
      // jika bola berada di rentang paddleTwo
  ...
  function playerCollisionEvent() {
    ...
    const maxTopSlope = (courtTopRightY - (paddleOne.y - paddleOne.height))/(courtTopRightX - (paddleOne.x + paddleOne.width)); // rentang dari -0.38 sampai 0
    const maxBottomSlope = (courtBottomRightY - (paddleOne.y + paddleOne.height))/(courtBottomRightX - (paddleOne.x + paddleOne.width)); // rentang dari 0 sampai 0.54
  ...
  function computerCollisionEvent() {
    ...
    const maxTopSlope = (courtTopLeftY - (paddleTwo.y - paddleTwo.height))/(courtTopLeftX - (paddleTwo.x - paddleTwo.width)); // 0 sampai 0.56
    const maxBottomSlope = (courtBottomLeftY - (paddleTwo.y + paddleTwo.height))/(courtBottomLeftX - (paddleTwo.x - paddleTwo.width)); // 0 sampai -0.39
  ...
  function didHitWall() {
    ...
    // pantulkan bola jika mengenai batas atas atau bawah
  ...
  function didScore() {
    ...
    // jika bola melewati batas kiri atau kanan, tambahkan skor dan reset bola
  ...
  function drawCourtLines() {
    ...
    // garis vertikal di tengah lapangan
  ...
  function drawScore() {
    ...
    fill(54);
    textAlign(RIGHT);
    text(leftPlayerScore, padding + quadOffset + (courtWidth * 0.25), padding - 10); // skor pemain
    textAlign(LEFT);
    text(rightPlayerScore, width - padding - (courtWidth * 0.25), padding - 10); // skor komputer
  }
